  ...
  return (
    <div className="event-card bg-gray-800 p-4 rounded-lg shadow-lg">
      ...
      <div className="event-details">
        ...
        <p className="event-date text-sm text-gray-400 mb-1">{new Date(event.date).toLocaleDateString('en-GB', options)}</p>
        ...
        <a href={event.link} target="_blank" rel="noopener noreferrer" className="event-link text-purple-500 hover:underline">
          Learn More
        </a>

        <button
          onClick={() => toggleFavorite(`${event.id}-${event.location ? 'offline' : 'online'}`)}
          className={`favorite-btn ml-4 mt-4 p-2 rounded-full transition-colors duration-300 ${
