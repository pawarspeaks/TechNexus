...
const EventsPage = () => {
    ...
    useEffect(() => {
        const fetchEvents = async () => {
            const response = await fetch('/data/offline-events.json');
            if (!response.ok) {
    ...
    const toggleFavourite = (event: any) => {
        ...
        const eventType = 'offline'; // Specify the event type
        ...
        <section className="events  min-h-screen flex flex-col items-center">
            <div className="px-4 py-8 w-full max-w-4xl">
                ...
                <p className="text-base md:text-lg text-center text-slate-100 mb-4">
                    Forge Real Connections: Meet, Network and Innovate In-person Tech Events!
                </p>
                                    ...
                                    <div className="py-4 grid grid-cols-2 gap-5">
                                        ...
                                        <div className="">
                                            ...
                                            <button onClick={() => toggleFavourite(event)} className="mt-4">
                                                <Heart className={`w-6 h-6 ${favourites.some(fav => fav.event?.id === event.id && fav.type === 'offline') ? 'text-red-500' : 'text-gray-500'}`} />
                                            </button>
                                        </div>
                                    </div>
                                </div>
